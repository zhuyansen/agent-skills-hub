  ...
  const rows = [
    {
      ...
      render: (s: SkillDetail) => s.score?.toFixed(1) || "-",
    },
    {
      key: "stars",
      label: t("detail.stars"),
      render: (s: SkillDetail) => s.stars.toLocaleString(),
    },
    {
      key: "forks",
      label: t("detail.forks"),
      render: (s: SkillDetail) => s.forks.toLocaleString(),
    },
    {
      key: "issues",
      label: t("detail.issues"),
      render: (s: SkillDetail) => s.open_issues.toLocaleString(),
    },
    {
      key: "commits",
      label: t("detail.commits"),
      render: (s: SkillDetail) => s.total_commits.toLocaleString(),
    },
    {
      key: "followers",
      label: t("detail.followers"),
      render: (s: SkillDetail) => s.author_followers.toLocaleString(),
    },
    {
      key: "category",
      label: t("table.category"),
      render: (s: SkillDetail) => s.category,
    },
    {
      key: "language",
      label: "Language",
      render: (s: SkillDetail) => s.language || "-",
    },
    {
      key: "tokens",
      label: t("detail.tokens"),
      render: (s: SkillDetail) =>
        s.estimated_tokens > 0
          ? `~${(s.estimated_tokens / 1000).toFixed(0)}k`
          : "-",
    },
    {
      key: "quality",
      label: t("detail.qualityAnalysis"),
      render: (s: SkillDetail) => (s.quality_score || 0).toFixed(0),
    },
    {
      key: "completeness",
      label: t("detail.completeness"),
      render: (s: SkillDetail) =>
        ((s.quality_completeness || 0) * 100).toFixed(0) + "%",
    },
    {
      key: "clarity",
      label: t("detail.clarity"),
      render: (s: SkillDetail) =>
        ((s.quality_clarity || 0) * 100).toFixed(0) + "%",
    },
    {
      key: "agent",
      label: t("detail.agentReadiness"),
      render: (s: SkillDetail) =>
        ((s.quality_agent_readiness || 0) * 100).toFixed(0) + "%",
    },
  ];
    ...
    skills.forEach((s, i) => {
      ...
      switch (key) {
        ...
        case "commits":
          ...
          break;
        case "followers":
          val = s.author_followers;
          break;
        case "quality":
          val = s.quality_score || 0;
          break;
        case "completeness":
          val = s.quality_completeness || 0;
          break;
        case "clarity":
          val = s.quality_clarity || 0;
          break;
        case "agent":
          val = s.quality_agent_readiness || 0;
          break;
        case "tokens":
          val = s.estimated_tokens;
          break;
      }
      if (val > bestVal) {
        bestVal = val;
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <Helmet>
        ...
        <meta
          name="description"
          content={`Side-by-side comparison of ${skills.map((s) => s.repo_name).join(" vs ")}: security grade, quality score, stars, and freshness — on Agent Skills Hub.`}
        />
      </Helmet>
      ...
      <main className="max-w-5xl mx-auto px-4 py-6">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5 mb-6">
          ...
          <div
            className={`grid gap-4 ${skills.length === 2 ? "grid-cols-2" : skills.length === 3 ? "grid-cols-3" : "grid-cols-4"}`}
          >
            {skills.map((s) => (
