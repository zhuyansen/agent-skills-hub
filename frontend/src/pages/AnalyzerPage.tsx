...
> = {
  ...
  2: {
    label: "Known Security Team",
    labelZh: "知名安全团队",
    color: "text-blue-600 dark:text-blue-400",
    icon: "🛡️",
  },
  3: {
    label: "High-Star + Licensed",
    labelZh: "高星 + 有许可证",
        ...
        try {
          ...
          if (sb) {
            const { data } = await sb
              .from("skills")
              .select(
                "category,quality_score,quality_completeness,quality_clarity,quality_specificity,quality_examples,quality_agent_readiness,security_grade,security_flags",
              )
            ...
            if (data) {
              ...
              try {
                existingFlags = JSON.parse(data.security_flags || "[]");
              } catch {
                /* ignore */
              }
            }
          ...
          /* Supabase not available, continue */
        }
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
      <Helmet>
        ...
        <meta
          ...
          content={
            isZh
              ? "粘贴任意 GitHub 仓库地址，立即获得安全评级、红旗信号和质量分。免费、无需注册，覆盖 186,000+ 个 AI agent skill 与 MCP server。"
              : "Paste any GitHub repo URL to get an instant security grade, red flags, and quality score. Free, no signup — covering 186,000+ AI agent skills & MCP servers."
          }
        />
      </Helmet>

      <SiteHeader
        breadcrumb={[{ label: isZh ? "安全分析器" : "Security Analyzer" }]}
      />
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400 text-xs font-medium mb-4">
            ...
            {isZh
              ? "SlowMist 安全框架驱动 · 11 类红旗检测"
              : "SlowMist Framework · 11 Red-Flag Categories"}
          </div>
    ...
    <div className="space-y-6">
      ...
      <div className={`${gc.bg} border ${gc.border} rounded-xl p-6`}>
        <div className="flex items-start gap-4">
          <div
            className={`w-16 h-16 rounded-full border-4 ${gc.border} flex items-center justify-center shrink-0`}
          >
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-3 mb-1 flex-wrap">
              ...
              {result.indexed && (
                <span className="px-2 py-0.5 text-[10px] font-medium rounded bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600">
                  {isZh ? "已收录" : "Indexed"}
                </span>
              )}
            </div>
            <p className="text-sm text-gray-600 dark:text-gray-300">
              {scan.flags.length === 0
                ...
                : isZh
                  ? `检测到 ${scan.flags.length} 个安全标记，请查看下方详细报告。`
                  : `${scan.flags.length} security flag(s) detected. See detailed report below.`}
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="mb-4 p-3 rounded-lg bg-gray-50 dark:bg-gray-800 flex items-center gap-3">
          ...
          <div>
            ...
            <p className={`text-sm font-bold ${tc.color}`}>
              Tier {scan.trustTier}: {isZh ? tc.labelZh : tc.label}
            </p>
          </div>
        ...
        {scan.flagDetails.length > 0 ? (
          <div className="space-y-2">
            {scan.flagDetails.map((flag: FlagDetail, i: number) => (
              <div
                key={i}
                className="flex items-start gap-2 p-2.5 rounded-lg bg-gray-50 dark:bg-gray-800"
              >
                <span
                  className={`px-1.5 py-0.5 text-[10px] font-bold rounded shrink-0 ${SEVERITY_COLORS[flag.severity] || SEVERITY_COLORS.medium}`}
                >
          ...
          <div className="p-4 rounded-lg bg-green-50 dark:bg-green-900/20 text-center">
            <span className="text-green-600 dark:text-green-400 text-sm font-medium">
              ...
              {isZh
                ? "未检测到危险模式 (11 类红旗均通过)"
                : "No dangerous patterns detected (all 11 red-flag categories passed)"}
            </span>
          </div>
        )}
      </div>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-5">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
            {[
              ...
              {
                key: "agent_readiness",
                label: isZh ? "Agent 就绪" : "Agent Ready",
              },
            ].map(({ key, label }) => {
              const val = quality[key as keyof typeof quality] ?? 0;
      ...
      <div className="text-center text-[11px] text-gray-400 dark:text-gray-500">
        ...
        {" · "}
        <a
          href="https://github.com/slowmist/slowmist-agent-security"
          target="_blank"
          rel="noopener noreferrer"
