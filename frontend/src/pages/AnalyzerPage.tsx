...
import type { AnalyzerResult, FlagDetail } from "../types/analyzer";
...
export function AnalyzerPage() {
  ...
  const handleScan = useCallback(async (e?: React.FormEvent) => {
    ...
    try {
      ...
      setResult(data);
      saveHistory({ url, grade: data.security.final_grade, time: Date.now() });
    } catch (err: any) {
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400 text-xs font-medium mb-4">
            ...
            {isZh ? "SlowMist 安全框架驱动" : "Powered by SlowMist Security Framework"}
          </div>
          ...
          <p className="text-gray-500 dark:text-gray-400 max-w-lg mx-auto">
            {isZh
              ? "粘贴任何 GitHub 仓库 URL，即时分析安全风险。11 类红旗模式 + 5 级信任体系 + AI 深度分析。"
              : "Paste any GitHub repo URL for instant security analysis. 11 red-flag categories + 5-tier trust hierarchy + AI deep analysis."}
          </p>
        </div>
...
function AnalyzerResults({ result, isZh }: { result: AnalyzerResult; isZh: boolean }) {
  ...
  const trustTier = security.trust_tier || 5;
  const trustConfig = TRUST_TIER_CONFIG[trustTier] || TRUST_TIER_CONFIG[5];
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="mb-4 p-3 rounded-lg bg-gray-50 dark:bg-gray-800 flex items-center gap-3">
          ...
          <div>
            <span className="text-[10px] uppercase tracking-wider text-gray-400 font-medium">
              {isZh ? "信任层级" : "Trust Tier"}
            </span>
            <p className={`text-sm font-bold ${trustConfig.color}`}>
              Tier {trustTier}: {isZh ? trustConfig.labelZh : trustConfig.label}
            </p>
          </div>
          <div className="ml-auto flex gap-1">
            {[1, 2, 3, 4, 5].map(t => (
              <div
                key={t}
                className={`w-2 h-6 rounded-sm ${t <= trustTier ? (trustTier <= 2 ? "bg-green-400" : trustTier <= 3 ? "bg-blue-400" : trustTier <= 4 ? "bg-amber-400" : "bg-red-400") : "bg-gray-200 dark:bg-gray-700"}`}
              />
            ))}
          </div>
        </div>
      ...
      {/* Scan Details — Standardized Report */}
      <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-5">
        <h3 className="text-sm font-semibold text-gray-900 dark:text-white mb-4">
          {isZh ? "安全审查报告" : "Security Audit Report"}
        </h3>
        ...
        <div className="grid grid-cols-2 gap-4 mb-4">
          <div className="p-3 rounded-lg bg-gray-50 dark:bg-gray-800">
            <span className="text-[10px] uppercase tracking-wider text-gray-400 font-medium">
              {isZh ? "规则引擎 (11类红旗)" : "Rule Engine (11 Red Flags)"}
            </span>
        ...
        {/* Flag Details — Standardized findings table */}
        {security.flags.length > 0 && (
          <div className="mb-4">
            <span className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-2 block">
              {isZh ? "检测发现" : "Findings"} ({security.flags.length})
            </span>
            <div className="space-y-2">
              {(security.flag_details || security.flags.map(f => ({ name: f, severity: "medium" as const, description: f }))).map((flag: FlagDetail | string, i: number) => {
                const detail = typeof flag === "string" ? { name: flag, severity: "medium", description: flag } : flag;
                return (
                  <div key={i} className="flex items-start gap-2 p-2.5 rounded-lg bg-gray-50 dark:bg-gray-800">
                    <span className={`px-1.5 py-0.5 text-[10px] font-bold rounded shrink-0 ${SEVERITY_COLORS[detail.severity] || SEVERITY_COLORS.medium}`}>
                      {detail.severity.toUpperCase()}
                    </span>
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-medium text-gray-700 dark:text-gray-300">{detail.description}</p>
                      <p className="text-[10px] text-gray-400 dark:text-gray-500 font-mono mt-0.5">{detail.name}</p>
                    </div>
                  </div>
                );
              })}
        ...
        {security.flags.length === 0 && (
          <div className="p-4 rounded-lg bg-green-50 dark:bg-green-900/20 text-center">
            <span className="text-green-600 dark:text-green-400 text-sm font-medium">
              ✓ {isZh ? "未检测到危险模式" : "No dangerous patterns detected"}
            </span>
          </div>
        )}
        ...
        {security.llm_analysis?.findings && security.llm_analysis.findings.length > 0 && (
          <div className="mt-4">
            <span className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-2 block">
              {isZh ? "AI 深度发现" : "AI Deep Findings"}
            </span>
            <div className="space-y-2">
              {security.llm_analysis.findings.map((finding, i) => (
                <div key={i} className="flex items-start gap-2 p-2.5 rounded-lg bg-gray-50 dark:bg-gray-800">
                  <span className={`px-1.5 py-0.5 text-[10px] font-bold rounded shrink-0 ${SEVERITY_COLORS[finding.severity] || SEVERITY_COLORS.low}`}>
      ...
      <div className="text-center text-[11px] text-gray-400 dark:text-gray-500">
        {isZh
          ? "安全框架灵感来源: SlowMist Agent Security Framework"
          : "Security framework inspired by SlowMist Agent Security Framework"}
        {" · "}
        <a href="https://github.com/slowmist/slowmist-agent-security" target="_blank" rel="noopener noreferrer" className="hover:text-indigo-500">
          GitHub →
        </a>
      </div>
    </div>
