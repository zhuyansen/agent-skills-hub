...
export async function fetchTrendingWeeks(): Promise<
  import("../types/skill").TrendingWeek[]
> {
  if (USE_SUPABASE) return sbFetchTrendingWeeks();
  return request<import("../types/skill").TrendingWeek[]>(
    "/api/trending/weeks",
    { cache: "no-store" },
  );
}
...
export async function fetchTrendingHistory(
  ...
  return request<import("../types/skill").WeeklyTrendingEntry[]>(
    `/api/trending/history?week_start=${weekStart}`,
    { cache: "no-store" },
  );
}
...
export async function fetchQuickSearch(
  ...
  if (USE_SUPABASE) {
    ...
    const { data, error } = await sb
      .from("skills")
      .select(
        "id,repo_full_name,repo_name,repo_url,description,author_name,author_avatar_url,stars,score,category,language,platforms,size_category,repo_size_kb,author_followers,forks,open_issues,total_issues,total_commits,topics,license,star_momentum,project_type,last_commit_at,created_at,last_synced,quality_completeness,quality_clarity,quality_specificity,quality_examples,quality_agent_readiness,quality_score,readme_size,readme_structure_score,estimated_tokens,homepage_url",
      )
      .or(
  ...
  return request<Skill[]>(
    `/api/skills?search=${encodeURIComponent(query)}&page_size=${limit}&sort_by=score&sort_order=desc`,
  ).then((r: Skill[] | { items: Skill[] }) => ("items" in r ? r.items : r));
}
...
export async function fetchSkillBySlug(slug: string): Promise<SkillDetail> {
  ...
  return request<SkillDetail>(
    `/api/skills/by-slug/${encodeURIComponent(slug)}`,
  );
}

export async function fetchLanguageStats(): Promise<
...
export async function fetchOrgBuilders(): Promise<
  import("./supabaseClient").OrgBuilder[]
> {
...
export async function analyzeRepo(
  repoUrl: string,
): Promise<import("../types/analyzer").AnalyzerResult> {
...
export async function adminUpdateMaster(
  ...
  if (USE_SUPABASE) {
    ...
    if (data.github_aliases !== undefined)
      payload.github_aliases = JSON.stringify(data.github_aliases);
    ...
    if (data.x_posts_count !== undefined)
      payload.x_posts_count = data.x_posts_count;
...
export async function adminDeleteMaster(
  token: string,
  id: number,
): Promise<void> {
  if (USE_SUPABASE) {
    await sbAdminAction(token, "delete_master", { id });
    return;
  }
  await request(`/api/admin/masters/${id}`, {
    method: "DELETE",
    headers: adminHeaders(token),
  });
}

export async function adminFetchExtraRepos(
  token: string,
): Promise<ExtraRepoData[]> {
  if (USE_SUPABASE)
    return sbAdminAction<ExtraRepoData[]>(token, "fetch_extra_repos");
  return request<ExtraRepoData[]>("/api/admin/extra-repos", {
    headers: adminHeaders(token),
  });
}

export async function adminCreateExtraRepo(
  token: string,
  full_name: string,
): Promise<ExtraRepoData> {
  if (USE_SUPABASE)
    return sbAdminAction<ExtraRepoData>(token, "create_extra_repo", {
      full_name,
    });
  return request<ExtraRepoData>("/api/admin/extra-repos", {
    method: "POST",
...
export async function adminDeleteExtraRepo(
  token: string,
  id: number,
): Promise<void> {
  if (USE_SUPABASE) {
    await sbAdminAction(token, "delete_extra_repo", { id });
    return;
  }
  await request(`/api/admin/extra-repos/${id}`, {
    method: "DELETE",
    headers: adminHeaders(token),
  });
}

export async function adminApproveExtraRepo(
  token: string,
  id: number,
): Promise<{ message: string }> {
  if (USE_SUPABASE)
    return sbAdminAction<{ message: string }>(token, "approve_extra_repo", {
      id,
    });
  return request(`/api/admin/extra-repos/${id}/approve`, {
    method: "PUT",
    headers: adminHeaders(token),
  });
}

export async function adminRejectExtraRepo(
  token: string,
  id: number,
): Promise<{ message: string }> {
  if (USE_SUPABASE)
    return sbAdminAction<{ message: string }>(token, "reject_extra_repo", {
      id,
    });
  return request(`/api/admin/extra-repos/${id}/reject`, {
    method: "PUT",
    headers: adminHeaders(token),
  });
}

export async function adminFetchSearchQueries(
  token: string,
): Promise<SearchQueryData[]> {
  if (USE_SUPABASE)
    return sbAdminAction<SearchQueryData[]>(token, "fetch_search_queries");
  return request<SearchQueryData[]>("/api/admin/search-queries", {
    headers: adminHeaders(token),
  });
}

export async function adminCreateSearchQuery(
  token: string,
  query: string,
): Promise<SearchQueryData> {
  if (USE_SUPABASE)
    return sbAdminAction<SearchQueryData>(token, "create_search_query", {
      query,
    });
  return request<SearchQueryData>("/api/admin/search-queries", {
    method: "POST",
...
export async function adminDeleteSearchQuery(
  token: string,
  id: number,
): Promise<void> {
  if (USE_SUPABASE) {
    await sbAdminAction(token, "delete_search_query", { id });
    return;
  }
  await request(`/api/admin/search-queries/${id}`, {
    method: "DELETE",
    headers: adminHeaders(token),
  });
}
...
export async function adminFetchSubscribers(
  ...
  if (USE_SUPABASE)
    return sbAdminAction<SubscriberData[]>(token, "fetch_subscribers");
  return request<SubscriberData[]>("/api/admin/subscribers", {
    headers: adminHeaders(token),
  });
}
...
export async function adminDeleteSubscriber(
  ...
  if (USE_SUPABASE) {
    await sbAdminAction(token, "delete_subscriber", { id });
    return;
  }
  await request(`/api/admin/subscribers/${id}`, {
    method: "DELETE",
    headers: adminHeaders(token),
  });
}

export async function adminFetchSyncLogs(
  token: string,
): Promise<SyncLogData[]> {
  if (USE_SUPABASE)
    return sbAdminAction<SyncLogData[]>(token, "fetch_sync_logs");
  return request<SyncLogData[]>("/api/admin/sync-logs", {
    headers: adminHeaders(token),
  });
}
...
export async function adminTriggerSync(
  token: string,
): Promise<{ message: string; sync_id: number }> {
  if (USE_SUPABASE)
    return sbAdminAction<{ message: string; sync_id: number }>(
      token,
      "trigger_sync",
    );
...
export async function adminFetchSkills(
  ...
  if (USE_SUPABASE)
    return sbAdminAction<Skill[]>(token, "fetch_skills", {
      page,
      search: search ?? "",
    });
  const sp = new URLSearchParams({ page: String(page), page_size: "50" });
  if (search) sp.set("search", search);
  return request<Skill[]>(`/api/admin/skills?${sp}`, {
    headers: adminHeaders(token),
  });
}
...
export async function adminDeleteSkill(
  ...
  if (USE_SUPABASE) {
    await sbAdminAction(token, "delete_skill", { id });
    return;
  }
  await request(`/api/admin/skills/${id}`, {
    method: "DELETE",
    headers: adminHeaders(token),
  });
}
...
export async function adminFetchMasterApplications(
  token: string,
): Promise<MasterApplicationData[]> {
  if (USE_SUPABASE)
    return sbAdminAction<MasterApplicationData[]>(
      token,
      "fetch_master_applications",
    );
  return request<MasterApplicationData[]>("/api/admin/master-applications", {
    headers: adminHeaders(token),
  });
}

export async function adminApproveMasterApplication(
  token: string,
  id: number,
): Promise<{ message: string }> {
  if (USE_SUPABASE)
    return sbAdminAction<{ message: string }>(
      token,
      "approve_master_application",
      { id },
    );
  return request(`/api/admin/master-applications/${id}/approve`, {
    method: "PUT",
    headers: adminHeaders(token),
  });
}

export async function adminRejectMasterApplication(
  token: string,
  id: number,
): Promise<{ message: string }> {
  if (USE_SUPABASE)
    return sbAdminAction<{ message: string }>(
      token,
      "reject_master_application",
      { id },
    );
  return request(`/api/admin/master-applications/${id}/reject`, {
    method: "PUT",
    headers: adminHeaders(token),
  });
}
