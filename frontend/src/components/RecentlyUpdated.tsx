  ...
  return (
    <section className="mb-10">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h2 className="text-lg font-bold text-gray-900 dark:text-gray-100">{t("recentlyUpdated.title")}</h2>
        <span className="text-sm text-gray-400 dark:text-gray-500 hidden sm:inline">{t("recentlyUpdated.subtitle")}</span>
      </div>
      <div className="bg-white dark:bg-gray-800/80 border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden divide-y divide-gray-50 dark:divide-gray-800">
        {items.map((skill, i) => (
          <div
            ...
            className={`flex items-center gap-3 px-4 py-3 hover:bg-green-50/40 dark:hover:bg-gray-800 transition-colors cursor-pointer group${i >= 4 && !expanded ? " hidden sm:flex" : ""}`}
          >
            {/* Avatar */}
            <img
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                ...
                {skill.language && (
                  <span className="px-1.5 py-0.5 text-[10px] rounded bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 hidden sm:inline">
                    {skill.language}
                  </span>
                )}
              </div>
              <span className="text-xs text-gray-400 dark:text-gray-500 truncate block">{skill.author_name}</span>
            </div>
