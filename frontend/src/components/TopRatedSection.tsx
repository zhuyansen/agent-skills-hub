  ...
  return (
    <section className="mb-10">
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="text-sm text-gray-400">{t("topRated.subtitle")}</span>
        <span className="relative group">
          <svg className="w-4 h-4 text-gray-300 cursor-help" fill="currentColor" viewBox="0 0 20 20">
            <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" />
          </svg>
          <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-2 text-xs text-white bg-gray-800 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none z-10">
            {t("topRated.tooltip")}
          </span>
        </span>
      </div>
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden divide-y divide-gray-50">
        {items.map((skill, i) => (
          <div
            ...
            className="flex items-center gap-3 px-4 py-3 hover:bg-blue-50/40 transition-colors cursor-pointer group"
          >
            <span className={`text-sm font-bold w-6 text-center ${i < 3 ? "text-blue-600" : "text-gray-400"}`}>
              {i + 1}
            </span>
            <ScoreBadge score={skill.score} size="sm" showTier />
            <img src={skill.author_avatar_url} alt="" className="w-7 h-7 rounded-full border border-gray-100" />
            <div className="flex-1 min-w-0">
              <span
                ...
                className="font-medium text-sm text-gray-900 hover:text-blue-600 hover:underline transition-colors truncate block cursor-pointer"
              >
                {skill.repo_name}
              </span>
            ...
            <span className="text-xs text-gray-400 w-14 text-right hidden sm:block">
