    ...
    <section className="mb-10">
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="text-sm text-gray-400 dark:text-gray-500">
          {t("newThisWeek.subtitle")}
        </span>
        {/* /daily/ is a static page — hard link, not <Link> */}
        <a
          href="/daily/"
          className="ml-auto text-sm font-medium text-indigo-600 dark:text-indigo-400 hover:underline shrink-0"
        >
          {lang === "zh" ? "每日榜单 →" : "Daily Top 10 →"}
        </a>
      </div>
      <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden divide-y divide-gray-50 dark:divide-gray-800">
        {items.map((skill, i) => (
          ...
          >
            <span
              className={`text-sm font-bold w-6 text-center ${i < 3 ? "text-green-600" : "text-gray-400 dark:text-gray-500"}`}
            >
              {i + 1}
            </span>
            <img
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1.5">
                <span className="font-medium text-sm text-gray-900 dark:text-gray-100 hover:text-blue-600 hover:underline transition-colors truncate cursor-pointer">
                  {skill.repo_name}
                </span>
