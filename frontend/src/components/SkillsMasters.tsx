...
interface Props {
  prefetchedMasters?: Master[];
  prefetchedOrgs?: OrgBuilder[];
}
...
export function SkillsMasters({
  ...
  const { t } = useI18n();
  const [masters, setMasters] = useState<Master[]>(prefetchedMasters ?? []);
  const [orgs, setOrgs] = useState<OrgBuilder[]>(prefetchedOrgs ?? []);
  const [expandedOrgIdx, setExpandedOrgIdx] = useState<number | null>(null);
  const [loaded, setLoaded] = useState(!!(prefetchedMasters && prefetchedOrgs));

  // Sync prefetched data when it arrives
  useEffect(() => {
    if (prefetchedMasters) {
      setMasters(prefetchedMasters);
      setLoaded((l) => l || !!prefetchedOrgs);
    }
  }, [prefetchedMasters, prefetchedOrgs]);
  useEffect(() => {
    if (prefetchedOrgs) {
      setOrgs(prefetchedOrgs);
      setLoaded((l) => l || !!prefetchedMasters);
    }
  }, [prefetchedOrgs, prefetchedMasters]);

  useEffect(() => {
    // Skip fetching if we already have prefetched data
    if (prefetchedMasters && prefetchedOrgs) return;
    let cancelled = false;
    Promise.all([
      prefetchedMasters
      ...
      {curated.length > 0 && (
        <>
          <div className="mb-4">
            ...
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-1.5 ml-7">
              {t("masters.warning")}
            </p>
          </div>
              ...
              >
                      ...
                      <span className="text-xs text-gray-400 dark:text-gray-500 ml-2">
                        ...
                        {m.x_followers > 0 && (
                          <span className="ml-1 text-gray-400 dark:text-gray-500">
                            · {formatCount(m.x_followers)}{" "}
                            {t("masters.followers")}
                          </span>
                        )}
                        {m.x_posts_count > 0 && (
                          <span className="ml-1 text-gray-400 dark:text-gray-500">
                            · {formatCount(m.x_posts_count)}{" "}
                            {t("masters.posts")}
                          </span>
                        )}
                      </span>
                ...
                <div className="flex items-center gap-4 mb-3 text-sm">
                  ...
                  <span className="flex items-center gap-1 text-gray-600 dark:text-gray-300">
                    ...
                    <strong>
                      {m.total_stars >= 1000
                        ? `${(m.total_stars / 1000).toFixed(1)}k`
                        : m.total_stars}
                    </strong>{" "}
                    {t("masters.totalStars")}
                  </span>
                </div>
                ...
                {m.tags.length > 0 && (
                  <div className="flex flex-wrap gap-1 mb-3">
                    {m.tags.map((tag) => (
                      <span
                        key={tag}
                        className="px-2 py-0.5 text-[10px] rounded-full bg-indigo-50 dark:bg-indigo-900/30 text-indigo-500 dark:text-indigo-400 border border-indigo-100 dark:border-indigo-800"
                      >
                        {tag}
                      </span>
                    ))}
                ...
                <div className="space-y-1.5">
                  {m.top_repos.slice(0, 4).map((r) => (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center gap-2 px-3 py-2 rounded-lg bg-white/80 dark:bg-gray-900/50 border border-gray-100 dark:border-gray-700 hover:border-indigo-200 dark:hover:border-indigo-700 hover:bg-indigo-50/30 dark:hover:bg-indigo-900/20 transition-all text-sm"
                    >
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {orgs.map((o, i) => {
              ...
              const visibleRepos = (o.top_repos || []).filter(
                (r) => r.stars > 0,
              );
              return (
                <div
                ...
                >
                  ...
                  <div className="space-y-1">
                    {visibleRepos.slice(0, isExpanded ? 5 : 2).map((r) => (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex items-center justify-between px-2 py-1.5 rounded-md hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors text-xs"
                      >
                        <span className="font-medium text-gray-700 dark:text-gray-200 truncate">
                          {r.repo_name}
                        </span>
                        <span className="flex items-center gap-0.5 text-gray-400 dark:text-gray-500 shrink-0 ml-2">
                          <svg
                            className="w-3 h-3 text-amber-400"
                            fill="currentColor"
                            viewBox="0 0 24 24"
                  ...
                  {visibleRepos.length > 2 && (
                    ...
                    >
                      {isExpanded
                        ? t("masters.showLess")
                        : `+${visibleRepos.length - 2} ${t("masters.more")}`}
                    </button>
                  )}
                </div>
              );
