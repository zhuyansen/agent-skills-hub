...
async function fetchWorkflowsFromSupabase(): Promise<WorkflowData[]> {
  ...
  const results = await Promise.all(
    categories.map(async (cat) => {
      const {
        ...
        .from("skills")
        .select(
          "repo_name,repo_full_name,description,stars,score,quality_score,author_name",
          { count: "exact" },
        )
        .eq("category", cat)
  ...
  workflows.sort(
    (a, b) =>
      (WORKFLOW_META[a.id]?.sort_order ?? 99) -
      (WORKFLOW_META[b.id]?.sort_order ?? 99),
  );
  return workflows;
}
        ...
        {workflows.map((wf) => (
          ...
          >
              ...
              {wf.skills.map((skill, idx) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <svg
                      className="w-3.5 h-3.5 text-gray-300 dark:text-gray-600 group-hover:text-blue-400 transition-colors"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
            ...
            <div className="border-t border-gray-100 dark:border-gray-800 pt-3 mt-3">
              ...
              >
                {t("workflows.viewAll").replace(
                  "{count}",
                  wf.skill_count.toLocaleString(),
                )}{" "}
                {lang === "zh" ? wf.title_zh : wf.title_en}
                <svg
