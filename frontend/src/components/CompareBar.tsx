      ...
      <div className="max-w-7xl mx-auto px-4 py-3 flex items-center gap-3">
        ...
        <div className="flex items-center gap-2 flex-1 min-w-0 overflow-x-auto">
          ...
          {items.map((item) => (
            <div
              key={item.id}
              className="flex items-center gap-1.5 px-2.5 py-1.5 bg-blue-50 border border-blue-200 rounded-full text-xs shrink-0"
            >
              <img src={item.author_avatar_url} alt="" className="w-4 h-4 rounded-full" />
              <span className="text-blue-700 font-medium max-w-24 truncate">{item.repo_name}</span>
              <button
