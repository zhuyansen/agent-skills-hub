  ...
  return (
    <section className="hero-gradient -mx-4 px-4 pt-10 pb-8 sm:pt-14 sm:pb-10 mb-6">
      <div className="max-w-3xl mx-auto text-center relative z-10">
        {/* Main headline — always light text on dark bg */}
        ...
        <div className="relative max-w-2xl mx-auto mb-5" ref={containerRef}>
          <svg
            className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-500 z-10"
          ...
          <input
            ...
            className="w-full pl-12 pr-12 py-4 border border-gray-700 rounded-2xl bg-gray-800/80 text-gray-100 text-base shadow-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-shadow hover:shadow-xl backdrop-blur-sm"
          />
          {query && (
            <button
              onClick={() => { setQuery(""); setResults([]); setShowDropdown(false); }}
              className="absolute right-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-500 hover:text-gray-300 cursor-pointer z-10"
              aria-label="Clear"
            >
                ...
                <div className="max-h-72 overflow-y-auto">
                  {results.map((skill, i) => (
                    <div
                      ...
                      className={`flex items-center gap-3 px-4 py-3 cursor-pointer transition-colors ${i === activeIdx ? "bg-indigo-900/30" : "hover:bg-gray-700"}`}
                    >
                      <img src={skill.author_avatar_url} alt="" width={32} height={32} className="w-8 h-8 rounded-full shrink-0" />
                      <div className="flex-1 min-w-0 text-left">
                        <div className="flex items-center gap-2">
                          <span className="text-sm font-semibold text-gray-100 truncate">{skill.repo_name}</span>
                          <span className="text-xs text-gray-500 shrink-0">{skill.author_name}</span>
                        </div>
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 mb-8">
          <span className="text-xs text-gray-500 font-medium">{t("hero.trending")}:</span>
          {TRENDING_TAGS.map((tag) => (
            <button
              key={tag}
              onClick={() => handleTagClick(tag)}
              className="px-3 py-1 text-xs font-medium bg-gray-800/80 text-gray-300 rounded-full border border-gray-600 hover:bg-indigo-900/30 hover:text-indigo-400 hover:border-indigo-700 transition-all cursor-pointer"
            >
              {tag}
            </button>
          ))}
        ...
        <div className="flex items-center justify-center gap-6 sm:gap-10">
          <div className="text-center">
            <div className="text-xl sm:text-2xl font-bold text-gray-100">{totalSkills.toLocaleString()}+</div>
            <div className="text-xs text-gray-500 mt-0.5">Skills</div>
          </div>
          <div className="w-px h-8 bg-gray-700" />
          <div className="text-center">
            <div className="text-xl sm:text-2xl font-bold text-indigo-400">{mcpCount.toLocaleString()}+</div>
            <div className="text-xs text-gray-500 mt-0.5">MCP Servers</div>
          </div>
          <div className="w-px h-8 bg-gray-700" />
          <div className="text-center">
            <div className="text-xl sm:text-2xl font-bold text-blue-400">{claudeCount.toLocaleString()}+</div>
            <div className="text-xs text-gray-500 mt-0.5">Claude Skills</div>
          </div>
          <div className="w-px h-8 bg-gray-700 hidden sm:block" />
          <div className="text-center hidden sm:block">
            <div className="text-xl sm:text-2xl font-bold text-emerald-400">{agentCount.toLocaleString()}+</div>
            <div className="text-xs text-gray-500 mt-0.5">Agent Tools</div>
          </div>
        </div>
      </div>
    </section>
