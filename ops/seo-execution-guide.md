...
| 10e | **HARO/Featured 专家回复** | ⚠️降级(07-26) | 哥飞会圆桌实证"已被做烂"(单链 $2-3 万价值的时代结束,产出链接与常规价持平)——改为低优先级实验:试 2 次无果即撤,不进月度节奏 |
...
## 二之二、五个发力点(2026-07-28 定版)+ pSEO 实测数据

### 五个发力点(按"自建/长期" vs "买/短期"分)

| # | 发力点 | 铁律 | 我们的数据佐证 | 性质 |
|---|---|---|---|---|
| 1 | **关键词纪律** | 品牌词首页靠前、其他页靠后;**每页只打一个词**(品牌词除外) | `/best/ppt-presentation/` 单点打穿 ppt 簇 → 全站第一词(26 点击/pos 7.2);泛词堆页全灭 | 自建·即时 |
| 2 | **外链匀速** | 模拟真实收录节奏,**别集中爆发** | ⚠️ 我们踩过:客座 5 连同期上线,不够匀速 | 自建·长期 |
| 3 | **pSEO 克制** | 子页面宁少勿滥 | 85 个 /best/ = 315 点击;8,638 个 /audit/ = **0 点击** | 自建·长期 |
| 4 | **GitHub 项目引流** | 做能装的产品(MCP/CLI),不是做广告 | GitHub 引荐连涨 8 天(65→234);mcpservers.org DR68 收录靠的是 MCP server 不是网站 | 自建·长期 |
| 5 | **社媒引流** | 有效但**贵且短命**,停更即掉 | X 是第二大来源(683 访客/28天),但完全依赖持续投入 | 买·短期 |

**排序含义**:1 免费即时见效;2-4 是复利资产(慢但不掉);5 是租来的流量。**力气按 1→4 排,5 只做维持。**

### pSEO 实测(28 天 GSC,我们自己的大样本)

| 页型 | 生成量 | 有曝光页 | 点击 | **点击/页** |
|---|--:|--:|--:|--:|
| **`/best/` 场景页** | **85** | 43 | **315** | **7.33** |
| `/skill/` 详情页 | 2,360 | 348 | 53 | 0.15 |
| `/author/` 作者页 | 500 | 63 | 3 | 0.05 |
| **`/audit/` 审计页** | **8,638** | **1** | **0** | **0.00** |

**85 页吃掉全站 73% 点击;8,638 页颗粒无收**,且后者占 sitemap 73%,是"已抓取未收录 305 + 已发现未收录 422"最可能的元凶。

### pSEO 四条铁律(从上表长出来)

1. **先有词,再有页** —— `/best/ppt-presentation/` 是先在 GSC 看到 ppt 4,230 曝光才建;反例 `/audit/` 是"有多少数据建多少页"。
2. **每页必须有独占信息(Information Gain)** —— 聚合/排序/对比/评级别处查不到;单条记录套模板 = 换词不换信息,正撞 2026-03「规模化内容滥用」(**不再区分是否 AI 生成**)。
3. **门槛闸 + 分批** —— skill 页 stars≥50、audit 页改「≥500★ 或 有红旗」;新页型 >1,000 页先放 5-10% 验收录质量。
4. **内链拓扑** —— 页量 >1,000 查孤儿岛;7,924 页曾是死水岛,接回主路才有排名资格。

### 跟谁学

- **中文**:哥飞(工具站 pSEO 最实,见 `ops/gefei-seo-playbook.md`)
- **经典案例**(拆模板用):**Zapier**(2.5万集成页)· **Wise**(货币对,每页有实时汇率)· **Canva**(模板页,每页有真素材)· **Nomad List**(城市页,独立开发者视角最近)
  —— 共同点:**每页都有别处拿不到的实时/结构化数据**,不是文字换词
- **英文实操派**:Ian Nuttall(公开 pSEO 实验数据)· Jon Yongfook(Bannerbear pSEO 复盘)
...
26. **静态壳页丢埋点 = 自然搜索转化"看不见"(2026-07-28,盲了数月)**:预渲染/静态壳的模板若只复制**带 src 的 script**,index.html 里那段**定义 `gtag()` 的内联引导**就漏了 —— 静态页加载了 GA 库但 `window.gtag` 永远 undefined,`trackEvent()` 静默 no-op。后果极隐蔽:**从静态页落地的整个会话零转化上报**(SPA 内跳不重载页面,盲区持续整场),而静态页正是**全部自然搜索落地**;首页入口的事件却一直正常(SPA 路由走 404.html 重定向回 index.html,带引导),**正常的那一半掩盖了坏掉的那一半**。判别法:生产页面 Console 跑 `typeof window.gtag` —— 首页 `function`、静态页 `undefined` 即中招;或 `curl 静态页 | grep -c "function gtag"` 得 0。修:资产提取正则额外抓含 `gtag(` 的内联脚本。⚠️ 通用教训:**任何"从 index.html 抽资产注入模板"的预渲染方案,都要显式清点内联脚本**(埋点、consent、feature flag 引导全在内联块里);且**双统计源的差异要当信号查**(Plausible 脚本在静态页里是全的,GA 不全 → 两边差值早就在提示这件事,我们当成了机器流量噪声)。

27. **两套建页策略 = 埋点静默掉队(2026-07-29,盲了 4 个月)**:静态页生成器有两种建法 —— **A 类借用型**(从 index.html 抽 `<script src>` 注入)和 **B 类手写型**(自己拼 `<head>`)。埋点加进 index.html 时,**A 类白捡、B 类一无所知**。我们的 `/best/` 场景页(28 天拿下 315/434 点击,**全站 73%**)属 B 类 —— **GA 里连一个 pageview 都不存在**,`/author/` `/daily/` 连 Plausible 也没有。考古时间线:skill 生成器 03-09 诞生用 `extractAssetTags` 借用 ✅;scenario 生成器 03-27 诞生自建 head,分析脚本命中 0 ❌;03-31「全站加 Plausible」那次**手工逐个复制**,补了 scenario/audit、漏了 author/daily,**GA 从头到尾没人补**。

**为什么藏 4 个月**(三层掩盖,每层都很"合理"):①B 类有 Plausible —— 看源码"有分析脚本",不逐个数就以为齐了,**半吊子补丁比完全没有更能骗人**;②A 类数据正常 —— GA 里首页/skill 页都在动,整体看不出缺口;③**差值被误判** —— GA 与 Plausible 长期对不上,我归因成"机器流量噪声"。**教训:一个能被现成故事解释的异常,恰恰最该再挖一层。**

**判别法**(任何多模板静态站通用):逐页型 `curl <页> | grep -c "function gtag"`,**按页型建表比对**,别只测首页。**修法:消灭第二个真相源** —— 抽 `analyticsTags()` 到 shared-utils,所有手写模板统一调用,以后加埋点只改一处。**通用原则:手工同步 N 个模板必然漏 —— 只要出现"复制到每个模板"的动作,就该改成共用函数。**
